...
public class PinPointAutoSpecimen extends LinearOpMode
{
    ...
    enum StateMachine
    {
        ...
        PRE_PARK,
        PARK_ASCENT_1,
        PARKED, SCORE_SPECIMEN, DRIVE_TO_SAMPLE, PRESCORE_SPECIMEN, STRAFE_TO_SAMPLE, BACK_UP_TO_SAMPLE, OBSERVATION_ZONE,
    }
    // Positions and Measurements
    static final Pose2D Submersible = new Pose2D(DistanceUnit.MM, 590, 40, AngleUnit.DEGREES,  -150);
    static final Pose2D ScoreSpecimen = new Pose2D(DistanceUnit.MM, 450, 40, AngleUnit.DEGREES, -150);
    static final Pose2D PreSampleDrive = new Pose2D(DistanceUnit.MM, 450, -700, AngleUnit.DEGREES, 90);
    static final Pose2D StrafeToSample = new Pose2D(DistanceUnit.MM, 1280, -700, AngleUnit.DEGREES, 90);
    static final Pose2D BackUpToSample = new Pose2D(DistanceUnit.MM, 1280, -775, AngleUnit.DEGREES, 90);
    static final Pose2D ObservationZone = new Pose2D(DistanceUnit.MM, 190, -775, AngleUnit.DEGREES, 90);
        ...
        while (opModeIsActive())
        {
            ...
            switch (stateMachine) {
                ...
                case SCORE_PRELOAD:
                     ...
                     */
                    if (nav.driveTo(odo.getPosition(), Submersible, 0.4, 1.0))
                    {
                        telemetry.addLine("Score Specimen 1");
                    }
                ...
                case PRESCORE_SPECIMEN:
                    telemetry.addLine("YAAHHH!");
                    if (nav.driveTo(odo.getPosition(), Submersible, 0.4, 0))
                    {
                        this.m_slideArmSubsystem.slideArm(kSlideAutoScore);
                        telemetry.addLine("Line 223");
                    }
                    if (this.m_slideArmSubsystem.atTarget(kSlideAutoScore))
                    {
                ...
                case SCORE_SPECIMEN:
                    ...
                    {
                        if (nav.driveTo(odo.getPosition(), ScoreSpecimen, 0.6, 0))
                        {
                ...
                case DRIVE_TO_SAMPLE:  //drive to the Sample
                     ...
                     if (this.m_slideArmSubsystem.atTarget(kSlideArmCloseSample));
                     {
                        ...
                        this.m_intakePivotSubsystem.pivotIntake(kIntakePivotScore);
                        this.m_intakeWheelSubsystem.spinIntake(0.0);//Turn On intake
                    }
                    if (nav.driveTo(odo.getPosition(), PreSampleDrive, 0.4, 0))
                    {
                ...
                case STRAFE_TO_SAMPLE:
                    telemetry.addLine("YAAHHH!");
                    if (nav.driveTo(odo.getPosition(), StrafeToSample, 0.4, 0))
                    {
                        this.m_slideArmSubsystem.slideArm(kSlideAutoScore);
                        telemetry.addLine("Line 223");
                    }
                ...
                case BACK_UP_TO_SAMPLE:
                    telemetry.addLine("YAAHHH!");
                    if (nav.driveTo(odo.getPosition(), BackUpToSample, 0.4, 0))
                    {
                        this.m_slideArmSubsystem.slideArm(kSlideAutoScore);
                        telemetry.addLine("Line 223");
                    }
...
//                    if(nav.driveTo(odo.getPosition(), PreSampleDrive, 0.3, 0))
//                    {
...
//
//                case SCORE_SAMPLE_2:
//                    if (nav.driveTo(odo.getPosition(), Submersiable, 0.4, 0))
//                    {
...
//                }
//                if (nav.driveTo(odo.getPosition(), Strafe2Sample, 0.4, .5))
//                {
...
//                    telemetry.addLine("Yes!");
//                    if(nav.driveTo(odo.getPosition(), BackUpToSample, 0.3, 0))
//                    {
...
//                    this.m_intakeWheelSubsystem.spinIntake(-1.0);//Turn On intake
//                }
//                if (nav.driveTo(odo.getPosition(), ObservationZone, 0.4, .5))
//                {
